...
`skills[]`（`name/path/description/enabled/scope/interface`）。Cameo 在 Codex / API runtime 都显示
这些本地 Skill，并支持输入框 `/` 补全。Codex runtime 在 `turn/start` / `turn/steer` 的 `input[]`
中附加 `{ type:"skill", name, path }`；第三方 API 没有 Codex native `skill` wire type，所以 API
runtime 会读取对应 `SKILL.md` 并作为请求上下文发送。
