...
| `api_runtime.rs` | 818 | OpenAI-compatible API runtime：图片模型走 `/v1/images/generations` / `/v1/images/edits`，Codex-style 主模型走 `/v1/responses` + `image_generation` tool；处理 `b64_json` / `url` / `image_generation_call.result` 输出，复用 Board asset minting、右侧血缘布局、session timeline 与 `UnifiedEvent` |
...
- **聊天**：`ChatPanel.tsx` / `ChatInlineImage.tsx` / `Composer.tsx` / `GenSettingsMenu.tsx` /
  `StreamingStatus.tsx` —— 消息列表、流式指示器、composer 栏 + Codex Skill 选择 + 生成档位选择器（模型 / 智能 /
