...
Cameo 是一个**桌面 app**（Tauri 2 + React + PixiJS），默认把 OpenAI 的 **Codex CLI**（用户本机
已登录的那份）当作 stateful 的生图 / 改图 agent，也允许切到 **OpenAI-compatible API**
作为直接生图 / 改图 runtime（Images endpoints 或 Responses `image_generation` tool）。
Cameo 自己不做模型、不做编排、不卖 token —— 它只做**"指着图说话"**
的空间画布：一个文件夹 = 一个 Board，画布上每张图是一个 Placement，AI 出的图落在源图右侧 + 一条
血缘线。Codex 跑作子进程，通过 JSON-RPC over stdio 跟它对话；API runtime 由 Rust 直接请求
`/v1/images/*` 或 `/v1/responses`。两条路径都翻译成同一个 `UnifiedEvent` 事件流打到前端 store。
...
| 谁 | 管什么 |
|---|---|
| **Runtime（Codex / API）** | 图像生成 / 改图能力；Codex 额外持有对话连续性（stateful，会反问）|
| **App（Cameo）** | 制品 + 空间语义（文件身份、布局、文件夹同步、引用注入）|

**铁律**：App 不在 agent / workflow 层做事 —— 生成 / 理解交给当前 Runtime。不要用状态机或文档
去重建对话语义；连续性只在 Codex 模式存在，由 Codex session 持有。

### 1.2 三个真相源

1. **Folder（本地文件夹）** —— 制品（图片文件）的唯一真相，Codex 模式下**也是 Codex 的 cwd**。
2. **Board doc**（`<folder>/.cameo/board.json`）—— 文件夹的空间投影：Placement / Annotation /
   布局。可重新构造，不持有制品。
3. **Session**（`<folder>/.cameo/sessions/`）—— 消息时间线 JSONL；Codex 模式下还镜像 Codex
   持有的 `threadId`。
...
- **标记 = overlay-as-image**：用户在画布上的矢量标记（点 / 框 / 椭圆 / 涂抹）渲染成蒙层 PNG，
  与原图**两张一起**发给当前 runtime（不依赖结构化 mask API）。代价 = 产出是全新整图，所以非破坏 +
  血缘是必须的。
...
| Runtime | **Codex `app-server`**（默认）/ **OpenAI-compatible API**（可选）| Codex = stateful session；API = 直接生图 / 改图请求，共用 `UnifiedEvent` |
| 鉴权 | `~/.codex` ChatGPT 订阅 / 本地 API key | Codex token 仍由 Codex CLI 管；API key 保存在本地 `~/.cameo/config.json` |
...
- ❌ **在 App 里做多模型编排 / workflow 决策** → 直接 API runtime 只是一条替代传输路径，不扩展成模型调度层。
...
│  │  • Runtime drivers   │    │  • Canvas scene (PixiJS)         │  │
│  │    Codex / API       │    │  • Chat / Composer / Gallery     │  │
│  │  • Cameo image proto │    │                                  │  │
│  │  • Logging / Config  │    │                                  │  │
│  │  • Updater / Tray    │    │                                  │  │
│  └──────────┬───────────┘    └──────────────────────────────────┘  │
...
- **所有对外网络都从 Rust 出**：Codex sidecar（自带网络）、API runtime（`api_runtime.rs`）+
  一切产品请求经 `net.rs` 的带代理
...
| `lib.rs` | 181 | **Tauri Builder**：plugin（opener/dialog/notification/updater/process）+ 全局 state（BoardRegistry / CodexRegistry / ApiRegistry）+ command 注册 + 本地图片协议（`cameo://`）+ 代理远程图片协议（`cmnet://`）+ 窗口关闭→托盘逻辑 |
| `commands.rs` | 1393 | **IPC 桥**：workspace / board / asset / session / runtime 路由 / 生成档位（gen settings / model 列表）/ cloud 传输 / config / device / updater / clipboard 的所有命令实现。前端的每个 `ipc.xxx` 在这里有 1:1 对应 |
...
| 文件 | 行数 | 职责 |
|---|---:|---|
| `codex.rs` | 2737 | **核心**：spawn `codex app-server` → JSON-RPC `initialize` / `thread/start` / `turn/start`（显式下发 model/effort/serviceTier/summary/personality，可附加 native `skill` input）+ active-turn `turn/steer` → drain items → 转 `UnifiedEvent`。还负责 `model/list`、`skills/list`、per-Board 生成档位、以及**消息时间线权威落盘**（user 记录在 turn 开始、assistant 在 turn 结束写入 active session）。协议细节见 [`CODEX_PROTOCOL.md`](./CODEX_PROTOCOL.md) |
| `api_runtime.rs` | 818 | OpenAI-compatible API runtime：图片模型走 `/v1/images/generations` / `/v1/images/edits`，Codex-style 主模型走 `/v1/responses` + `image_generation` tool；处理 `b64_json` / `url` / `image_generation_call.result` 输出；API 模式下本地 Skill 会读取 `SKILL.md` 并作为请求上下文发送；复用 Board asset minting、右侧血缘布局、session timeline 与 `UnifiedEvent` |
...
user 发一条
  ├─ App 渲染选中的图为 clean PNG + overlay PNG（写到 `<folder>/.cameo/tmp/`）
  ├─ 图片模型：无参考图 → POST {baseUrl}/images/generations
  ├─ 图片模型：有参考图 → POST {baseUrl}/images/edits multipart(image/image[])
  ├─ 主模型 / Codex-style 网关 → POST {baseUrl}/responses + image_generation tool
  ├─ API 返回 b64_json / url / image_generation_call.result
  │    → import_bytes
  │    → 新 Asset + 新 Placement（在源右侧）+ 一条血缘
  └─ turnComplete
```

### 5.3 事件模型 —— `UnifiedEvent`

事件经 `runtime.rs::UnifiedEvent` 抽象（runtime-agnostic），目前有 Codex 和 API 两个适配。类型在
...
| Event | 谁处理 | 怎么处理 |
|---|---|---|
| `sessionInit` | chat store | 记 `threadId`（API 模式为空）+ `model` |
...
1. **Inactivity watchdog**（`src/lib/watchdog.ts`）—— 长时间没有"进度类"事件就认定卡死，
   ...
   sidecar 退出。
   Codex 模式同一个 active turn 里的后续用户输入走 `turn/steer`，不用本地屏蔽；只有 Codex 明确拒绝
...
- Codex 模式用 `~/.codex` 的 ChatGPT 订阅；Cameo 不请求、不读取 Codex token。
- API 模式使用 Settings → AI runtime 里的 `baseUrl/apiKey/model/size`，配置保存在本地
  `~/.cameo/config.json`。它不提供 Codex 的 stateful agent / tool stream / 反问能力，只负责直接生图 / 改图。
...
- **代理覆盖产品所有对外网络出口**，不止 Codex：
  ...
  - **API runtime** —— `api_runtime.rs` 用 reqwest 直连或按 Settings 代理访问第三方 API。
...
- **聊天**：`ChatPanel.tsx` / `ChatInlineImage.tsx` / `Composer.tsx` / `GenSettingsMenu.tsx` /
  `StreamingStatus.tsx` —— 消息列表、流式指示器、composer 栏 + 本地 Skill 选择和 `/` 补全 + 生成档位选择器（模型 / 智能 /
...
**隐私边界**
- Codex 模式下，图片由 Codex sidecar 自己访问模型，跟用户平时用 CLI 一样。
- API 模式下，当前 turn 的 prompt、参考图和标记 overlay 会按用户配置上传到第三方 API endpoint。
...
| `meta.json` | `BoardMeta`：threadId（Codex 模式）/ 显示名 / active session / **per-Board 生成档位（gen_model/gen_effort/gen_service_tier，additive Option）** | `board.rs` + `storage.rs` |
| `sessions.json` | active session id + `SessionMeta[]` | `session.rs` |
| `sessions/<id>.jsonl` | 消息时间线（`ChatMessage` JSON）。**由 runtime 权威写入**，绑定 turn 的 session、不依赖前端聚焦（修复了旧版前端 best-effort 落盘可能静默丢历史的问题）| `codex.rs` / `api_runtime.rs` 写，`session.rs` IO |
| `thumbs/` | blake3-named 缩略图缓存 | `assets.rs` |
| `tmp/` | dispatch 临时图（clean + overlay）—— 放在 workspace 内，供 Codex 文件读取或 API multipart 上传 | `codex.rs` / `api_runtime.rs` |
...
| 目标用户 | 已付费 Codex 用户 + 需要第三方 image API 的用户 |
