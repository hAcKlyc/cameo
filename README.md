...
<p align="center">
  <b>An image-first canvas for your local Codex agent or API runtime.</b><br/>
  Point at images, mark them up, and talk to your selected runtime — generations and edits fan out across an infinite canvas.
</p>
...
**Cameo is a native desktop canvas that gives your image AI hands and eyes for image work.**
...
Cameo puts that capability into a **spatial canvas** instead. You open a folder, its images lay out on an infinite board, and you work the way you actually think about images: select one, draw a box on the part you mean, type an instruction, and the result lands **right next to the source** so the lineage is visible. In the default Codex runtime, the conversation stays continuous — "warmer," "now change the background," "do that to these three" — because there's one ongoing Codex session per board.

Cameo is **the hands and eyes, not the brain.** It does not host a model, sell tokens, or orchestrate agents. By default, generation and understanding stay with Codex using your existing ChatGPT subscription. If you prefer direct billing or a third-party gateway, Settings can switch the runtime to an OpenAI-compatible API. Cameo owns what a chat UI structurally can't: spatial layout, file identity, non-destructive lineage, and "what you're pointing at."

<p align="center">
  <img src="docs/app-en.png" width="840" alt="The Cameo desktop app — infinite canvas on the left, image runtime chat on the right" />
</p>
...
- **Runtime choice.** Use the local Codex CLI for a stateful agent session, or use an OpenAI-compatible API for direct image generation / edits.
- **One continuous session per board.** In Codex mode, Codex holds the conversation (it's stateful and can ask clarifying questions); Cameo mirrors the timeline. API mode keeps the same canvas and timeline, but is a direct image request rather than a full agent session.
...
- **Local Codex Skills** from the composer — pick them from the Skills menu or type `/` to autocomplete. Codex mode attaches protocol-level `skill` inputs; API mode expands the local `SKILL.md` as request context because third-party APIs have no native Codex Skill wire type.
- **OpenAI-compatible API runtime** for third-party gateways or direct API-key usage, including Images endpoints and Responses `image_generation`.
...
Cameo has two runtime paths:

- **Codex mode** runs a long-lived **`codex app-server`** sidecar (JSON-RPC 2.0 over stdio) — one process per board, so the session is genuinely stateful. Marked regions are flattened to an overlay image and sent alongside the clean image as file paths the agent reads itself. Local Codex Skills are listed with `skills/list` and attached to turns as native `skill` inputs. Image outputs (`imageGeneration` items, whether a saved path or base64) are copied into the board folder, minted as new content-addressed assets, and placed to the right of the source.
- **API mode** calls an OpenAI-compatible API from Rust. Image models use `POST /v1/images/generations` or `POST /v1/images/edits`; Codex-style main models use `POST /v1/responses` with the `image_generation` tool. Returned base64, URL, or `image_generation_call.result` images are imported into the same Board folder and placed with the same lineage rules.
...
- One image runtime:
  - **Codex mode:** Codex CLI installed and signed in: `codex login` (uses your ChatGPT subscription). Verify with `codex --version`. Cameo drives your own Codex — it is **not** bundled.
  - **API mode:** an OpenAI-compatible API key, base URL, and model. Configure these in Settings → AI runtime.
...
4. **Ask the selected runtime**: with image(s) selected, type an instruction (or hit a preset) and Send. A marked region is sent as an overlay.
5. The generated image appears **to the right of the source**. In Codex mode you can keep going in the same continuous session; in API mode each send is a direct image API call.
...
**v1 core loop built (dev).** The full path works end-to-end: open a folder → render on the canvas → mark a region → ask the selected runtime → output lands with lineage. Codex mode can continue the conversation; API mode sends direct image requests. Active development on branch `feat/v1-bootstrap`.
...
Cameo is an independent, unofficial tool. It can drive the Codex CLI or an OpenAI-compatible API, but is **not** affiliated with, endorsed by, or sponsored by OpenAI. "Codex" and related names belong to their respective owners.
...
**Cameo 是一个原生桌面画布，给你的图像 AI 装上做图用的「手」和「眼」。**

聊天框是处理图片最糟糕的界面。你没法指着照片的某个角说「就这块」；没法把三个变体并排着看；没法让原图和改后图挨在一起对照。Codex 本来就能生图、改图——但一条不断下滚的聊天列把这一切压扁成了一根你很快就跟丢的线索。

Cameo 把这份能力搬进了**空间画布**。你打开一个文件夹，里面的图在无限画布上铺开，然后用你真正思考图片的方式去工作：选中一张、在你想改的部位画个框、敲一句指令，结果就落在**源图右边**，血缘一目了然。在默认 Codex 运行时里，对话是连续的——「再暖一点」「现在换个背景」「把这三张都这么处理」——因为每个 board 对应一条持续的 Codex 会话。

Cameo 是**手和眼，不是脑子**。它不托管模型、不卖 token、不在 agent 层做编排。默认把生成与理解交给 Codex，用你已有的 ChatGPT 订阅；如果你想走直接计费或第三方网关，也可以在设置里切到 OpenAI-compatible API。Cameo 负责聊天 UI 结构上做不到的事：空间布局、文件身份、非破坏血缘，以及「你正指着什么」。

<p align="center">
  <img src="docs/app-zh.png" width="840" alt="Cameo 桌面应用——左边无限画布，右边图像运行时聊天" />
</p>
...
- **运行时可选。** 默认使用本机 Codex CLI 获得连续 agent 会话；也可以接 OpenAI-compatible API，直接调用第三方网关或 API key。
- **每个 board 一条连续会话。** 在 Codex 模式下，对话由 Codex 持有（有状态、会反问），Cameo 镜像时间线。API 模式保留同一套画布和时间线，但每次发送是直接生图 / 改图请求。
...
- **本地 Codex Skills**：可在输入框的 Skills 菜单选择，也可输入 `/` 自动补全。Codex 模式发送协议级 `skill` input；API 模式因为第三方接口没有 Codex Skill wire type，会把本地 `SKILL.md` 展开为请求上下文。
- **OpenAI-compatible API 运行时**：可接第三方网关或直接 API key，支持 Images endpoint 和 Responses `image_generation`。
...
- **Codex 模式**：长驻 **`codex app-server`** sidecar 运行（JSON-RPC 2.0 over stdio）——每个 board 一个进程，所以会话是真正有状态的。圈选区域会拍平成一张蒙层图，连同干净原图一起以文件路径的形式发给 agent 自读。本地 Codex Skills 通过 `skills/list` 获取，并作为原生 `skill` input 挂到 turn 上。图像产出（`imageGeneration`，无论是 savedPath 还是 base64）会被复制进 board 文件夹、铸成内容寻址的新 asset、落在源图右侧。
- **API 模式**：由 Rust 直接调用 OpenAI-compatible API。图片模型走 `POST /v1/images/generations` 或 `POST /v1/images/edits`；Codex 同款主模型走 `POST /v1/responses` + `image_generation` tool。API 返回的 base64、URL 或 `image_generation_call.result` 图片会进入同一套 Board 文件夹、内容寻址 asset 与血缘布局。
...
- 二选一配置图像运行时：
  - **Codex 模式**：Codex CLI 已安装并登录：`codex login`（用你的 ChatGPT 订阅）。用 `codex --version` 验证。Cameo 驱动你自己的 Codex——**不打包**它。
  - **API 模式**：准备 OpenAI-compatible API key、base URL 和模型名，在 Settings → AI runtime 里配置。
...
4. **问当前运行时**：选中图后输入指令（或点预设）发送。标记的区域会作为蒙层一起发出。
5. 生成图出现在**源图右侧**。Codex 模式下可以继续在同一条连续会话里改；API 模式下每次发送都是一次直接生图 / 改图请求。
...
**v1 核心闭环已实现（dev）。** 全链路跑通：开文件夹 → 渲染到画布 → 标记区域 → 问当前运行时 → 产出带血缘落地。Codex 模式可续聊；API 模式是直接生图 / 改图请求。开发分支 `feat/v1-bootstrap`。

## 许可与声明

采用 **[AGPL-3.0-or-later](./LICENSE)** 许可。你可以在其条款下使用、修改、再分发 Cameo；若你将修改版作为网络服务运行，AGPL 要求你向其用户提供源码。

Cameo 是独立的非官方工具。它可以驱动 Codex CLI，也可以调用 OpenAI-compatible API，但**不**隶属于 OpenAI，也未获其背书或赞助。「Codex」及相关名称归各自所有者所有。
