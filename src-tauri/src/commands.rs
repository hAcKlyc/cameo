...
pub async fn send_message(
    ...
    match crate::config::load().provider {
        ...
        crate::config::RuntimeProvider::Api => {
            if !skills.is_empty() {
                return Err("Codex Skills require the Codex runtime".into());
            }
            api_runtime::send_message(api.inner().clone(), board_id, text, sources, ov).await
        }
    }
}
...
pub async fn list_skills(
    ...
    match crate::config::load().provider {
        ...
        crate::config::RuntimeProvider::Api => Ok(Vec::new()),
    }
}
