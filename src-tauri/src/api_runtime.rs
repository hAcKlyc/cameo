...
pub async fn send_message(
    ...
    persist_user_record(&session.folder, &session.active_session(), &text, &source_placement_ids);
    let (placeholder_id, out_index) = start_generation(&session, &source_placement_ids);
    let prompt = build_api_prompt(&text, &refs);
...
fn build_api_prompt(text: &str, refs: &[ResolvedRef]) -> String {
    if refs.is_empty() {
    ...
    format!("Use the attached image(s) as visual reference.{overlay_hint}\n\n{cleaned}")
}
...
async fn generate_image(
    ...
    if prefers_responses_api(&api.model) {
        match call_responses_image(api, prompt, refs).await {
            Ok(image) => Ok(image),
            Err(responses_err) => call_image_api(api, prompt, refs)
                .await
                .map_err(|image_err| {
                    format!(
...
async fn call_responses_image(
    ...
    let resp = with_auth(api_client().post(endpoint(&api.base_url, "responses")), api)
        .json(&body)
        .send()
        .await
